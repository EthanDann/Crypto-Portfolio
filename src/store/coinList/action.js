...
export const getAllCoins = () => async (dispatch, getState) => {
  try {
    ...
    await axios
      .get(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${state.supportedCurrencies.activeCurrency}&order=market_cap_desc&per_page=10&page=${state.coins.pageNum}&sparkline=true&price_change_percentage=1h%2C24h%2C7d`
      )
      .then(({ data }) => {
        dispatch({
